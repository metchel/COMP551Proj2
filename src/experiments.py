...
cv = CountVectorizer(binary=True, analyzer='word', lowercase=True, stop_words='english', max_features=10000, ngram_range=(1, 2))
...
_end = time.time()
_diff = _end - _start
print('Training time (sklearn): {}'.format(_diff))
accuracy_train_3 = accuracy_score(train_y, nb.predict(train_features))
accuracy_validate_3 = accuracy_score(validate_y, nb.predict(validate_features))
"""

nb_2 = BernoulliNaiveBayes()
start = time.time()
nb_2.fit(train_features, train_y)
end = time.time()
diff = end - start
print('Training time: {}'.format(diff))

accuracy_train = accuracy_score(train_y, nb_2.predict(train_features))
accuracy_validate = accuracy_score(validate_y, nb_2.predict(validate_features))
